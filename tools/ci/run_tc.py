...
def setup_repository(args):
    ...
    # the branch changed since the decision task ran.
    ...
    # If it's not fetchable we fail the run
    # For a PR we are testing the provisional merge commit. If that's changed it
    # could be that the PR branch is updated or the base branch is updated. In the
    # former case we fail the run because testing an old commit is just a waste of
    ...
    if os.environ.get("GITHUB_PULL_REQUEST", "false") != "false":
        parents = run(["git", "show", "--no-patch", "--format=%P", "task_head"],
        ...
        if len(parents) == 2:
            ...
            commit = run(["git", "show", "--no-patch", "--format=%H", "task_head"],
